...
{
    internal class Stribog : IHashFunction
    {
        public string Name => "ГОСТ Р 34.11-2012";
        public string Authors => "Ионов, Сударкин";
        ...
        private readonly byte[] _iv = new byte[64];
        private byte[] _n = new byte[64];
        private byte[] _sigma = new byte[64];
        ...
        private byte[] AddModulo512(byte[] a, byte[] b) //Сложение по модулю 512
        {
        ...
        private byte[] S(byte[] state) //S - преобразование
        {
            byte[] result = new byte[64];
            for (int i = 0; i < 64; i++)
                result[i] = _sbox[state[i]];
            return result;
        }

        private byte[] P(byte[] state) //P - преобразование
        {
            byte[] result = new byte[64];
            for (int i = 0; i < 64; i++)
            {
        ...
        private byte[] L(byte[] state) //преобразование L - умножение на матрицу _a
        {
            byte[] result = new byte[64];
            for (int i = 0; i < 8; i++)
            {
        ...
        private byte[] KeySchedule(byte[] k, int i) //Преобразование Ki
        {
            k = AddXor512(k, _c[i]);
            k = S(k);
            k = P(k);
        ...
        private byte[] E(byte[] k, byte[] m) //Преобразование E
        {
        ...
        private byte[] G_n(byte[] n, byte[] h, byte[] m) //функция сжатия gn
        {
        ...
        {
            return GetHash(Encoding.Default.GetBytes(message));
        }
        ...
        public byte[] GetHash(byte[] message)
        {
            ...
            byte[] n512 = BitConverter.GetBytes(512);
            int inc = 0;
            while (len >= 512)
            {
                inc++;
            ...
            h = G_n(_n, h, paddedMes);
            byte[] mesLen = BitConverter.GetBytes(message1.Length * 8);
            _n = AddModulo512(_n, mesLen.Reverse().ToArray());
